...
FEATURE_DIR = Path(r"D:\CT_Brain_StudyWise_Project\features\convnext_tiny_multiwindow")
...
WINDOWS = [
    (40, 80),     # Brain window
    (80, 200),    # Subdural / blood-sensitive window
    (40, 380)     # Wide brain window
]
...
def read_multiwindow_dicom(dicom_path):
    ...
    for center, width in WINDOWS:
        win_img = apply_window(img, center, width)
        win_img = cv2.resize(win_img, (IMG_SIZE, IMG_SIZE))
        channels.append(win_img)

    multi_img = np.stack(channels, axis=-1)

    multi_img = multi_img * 255.0

    return multi_img.astype(np.float32)
...
def load_study_images(study_path):
    ...
    for dcm in dicom_files:
        try:
            img = read_multiwindow_dicom(dcm)
            images.append(img)
        except Exception:
            continue
...
def main():
    ...
    print("=" * 70)
    print("MULTI-WINDOW CONVNEXT FEATURE EXTRACTION")
    print("=" * 70)

    print("Windows used:")
    print("Channel 1: Brain window    WL=40, WW=80")
    print("Channel 2: Subdural window WL=80, WW=200")
    print("Channel 3: Wide window     WL=40, WW=380")
    ...
    for idx, row in df.iterrows():
        ...
        print(f"[{idx+1}/{len(df)}] Extracting multi-window features: {study_uid}")
    ...
    print("\nMulti-window feature extraction completed.")
